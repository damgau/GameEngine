 ...
 *
 * */
function BasicEnnemy() 
{
	this.name = "BasicEnnemy";
	this.enabled = true;
	this.started = false;
	this.rendered = true;
	this.fixedToCamera = true;

	this.spawnZone = new Circle(canvas.width/2, canvas.height/2, 300);
	...
	this.Start = function() 
	{
		if (!this.started) {
			// operation start
			// setPosition Random on circle
			var v = Math.Random.InCircle(this.spawnZone);
			this.SetPosition(v.x, v.y);
			this.Physics.Collider = new Circle( this.Transform.Position.x,
			...
			this.Transform.angle = Math.RadianToDegree(angle) + 180;
	...
	{
		// if (this.Transform.Position.x > window.innerWidth/2) this.Transform.RelativePosition.x --
		// if (this.Transform.Position.y > window.innerHeight/2) this.Transform.RelativePosition.y --
		// if (this.Transform.Position.x < window.innerHeight/2) this.Transform.RelativePosition.x ++
		// if (this.Transform.Position.y < window.innerHeight/2) this.Transform.RelativePosition.y ++
		//this.Transform.Position.x 
		//this.Transform.Position.y
		// Se déplace vers le centre (MainChar)
		...
		this.Physics.Collider = new Circle( this.Transform.Position.x + 50,
